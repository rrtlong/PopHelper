...
public class RcvAdapterWrapper extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final RecyclerView.ViewHolder viewHolder, int position) {
        ...
        if (type != TYPE_HEADER && type != TYPE_FOOTER && type != TYPE_EMPTY && type != TYPE_ERROR && type != TYPE_LOADING) {
            mWrapped.onBindViewHolder(viewHolder, position - getHeaderCount());
        }

        if ((type == TYPE_EMPTY || type == TYPE_ERROR || type == TYPE_LOADING) && emptyViewParent != null) {
            ...
            params.height = Math.max(emptyViewParent.getHeight() - headerHeight, DimensionsKt.dip(emptyViewParent, 150));
        }
    }
     ...
     *                        传null，则保持EmptyView的自有高度
     */
    public void setEmptyView(@Nullable View emptyView, @Nullable RecyclerView emptyViewParent, int type) {
        if (this.emptyView == emptyView) {
            return;
        }
        emptyType = type;
