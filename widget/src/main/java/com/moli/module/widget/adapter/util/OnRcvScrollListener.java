...
public class OnRcvScrollListener extends RecyclerView.OnScrollListener {
    ...
    protected static final int TYPE_LINEAR = 0;

    protected static final int TYPE_GRID = 1;

    protected static final int TYPE_STAGGERED_GRID = 2;
     ...
     * 触发在上下滑动监听器的容差距离
     */
    protected static final int HIDE_THRESHOLD = 20;
     ...
     * 是否需要监听控制
     */
    protected boolean mIsScrollDown = true;

    /**
     * Y轴移动的实际距离（最顶部为0）
     */
    protected int mScrolledYDistance = 0;

    /**
     * X轴移动的实际距离（最左侧为0）
     */
    protected int mScrolledXDistance = 0;
     ...
     * 判断layoutManager的类型
     */
    protected int judgeLayoutManager(RecyclerView.LayoutManager layoutManager) {
    ...
    protected int getOrientation(int type, RecyclerView.LayoutManager layoutManager) {
        int orientation = 1;
        switch (type) {
            case TYPE_LINEAR:
    ...
    protected int calculateFirstVisibleItemPos(int type, RecyclerView.LayoutManager layoutManager, int firstVisibleItemPosition) {
        switch (type) {
            case TYPE_LINEAR:
     ...
     * 计算当前是向上滑动还是向下滑动
     */
    protected void calculateScrollUpOrDown(int firstVisibleItemPosition, int dy) {
    ...
    public int getmLastVisibleItemPosition() {
